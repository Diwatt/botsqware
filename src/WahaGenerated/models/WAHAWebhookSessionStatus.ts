...
import type { WASessionStatusBody } from './WASessionStatusBody';
export type WAHAWebhookSessionStatus = {
    /**
    ...
    engine: WAHAWebhookSessionStatus.engine;
    /**
     * The event is triggered when the session status changes.
     */
    event: WAHAWebhookSessionStatus.event;
    payload: WASessionStatusBody;
    me?: MeInfo;
    environment: WAHAEnvironment;
};
export namespace WAHAWebhookSessionStatus {
    export enum engine {
        WEBJS = 'WEBJS',
        WPP = 'WPP',
        NOWEB = 'NOWEB',
